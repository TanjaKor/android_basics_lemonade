...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lemonImage!!.setOnClickListener {
            // clickLemonImage metodi kutsuu funktiota, joka määrittelee mitä tapahtuu
            // kun kuvaa klikataan
            clickLemonImage()
        }
        lemonImage!!.setOnLongClickListener {
            // showSnackbar näyttää pitkään painettaessa montako kertaa sitruunaa
            // on puristettu
            showSnackbar()
        }
    }
    ...
    private fun clickLemonImage() {
        ...
        if (lemonadeState == "select") {
            ...
            //kutsutaan pick-metodi lemontree luokasta
            lemonSize = lemonTree.pick()
            //määritetään puristusmäärä 0:ksi
            squeezeCount = 0
        } else if (lemonadeState == "squeeze") {
            //lisätään yksi puristus
            squeezeCount++
            //pienennetään sitruunan kokoa
            lemonSize--
            //jos tässä vaiheessa sitruunan koko on 0, vaihdetaan juo
            if (lemonSize == 0) {
                lemonadeState = DRINK
                lemonSize = -1
            }
        } else if (lemonadeState == "drink") {
            //muutetaan restarttiin
